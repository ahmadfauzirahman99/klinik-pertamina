...
class SSPF
{
    ...
    static function order($request, $columns, $isJoin = false)
    {
        ...
        if (isset($request['order']) && count($request['order'])) {
            $orderBy = array();
            $dtColumns = SSPF::pluck($columns, 'dt');
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    $binding = SSPF::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
     ...
     * in response to an SSPF request, or can be modified if needed before
    ...
    {
        $bindings = array();
        $db = SSPF::sql_connect($sql_details);

        // Build the SQL query string from the request
        $limit = SSPF::limit($request, $columns);
        $order = SSPF::order($request, $columns, $joinQuery);
        $where = SSPF::filter($request, $columns, $bindings, $joinQuery);
        ...
        if ($joinQuery) {
            $col = SSPF::pluck($columns, 'db', $joinQuery);
       ...
       $having
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `" . implode("`, `", SSPF::pluck($columns, 'db')) . "`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
       $having
        ...
        $data = SSPF::sql_exec($db, $bindings, $query);

        // Data set length after filtering
        $resFilterLength = SSPF::sql_exec(
            $db,
            "SELECT FOUND_ROWS()"
        );
        ...
        return array(
            ...
            "data"            => SSPF::data_output($columns, $data, $joinQuery)
        );
    }
    ...
    static function sql_exec($db, $bindings, $sql = null)
    {
        ...
        try {
            ...
            SSPF::fatal("An SQL error occurred: " . $e->getMessage());
        }
