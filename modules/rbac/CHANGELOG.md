...
- Enh: Custom side menu via `app\modules\rbac\Module::items` (mdmunir).
...
- Chg: Can customize return of `app\modules\rbac\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `app\modules\rbac\components\BizRule` (mdmunir).
...
- Chg: Remove `app\modules\rbac\Module::allowActions`. Set access control directly with `app\modules\rbac\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `app\modules\rbac\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
